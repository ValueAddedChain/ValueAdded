...
#include <valueAdd/json/Object.h>
#include <valueAdd/net/HTTPClient.h>
#include <valueAdd/protocol/JsonFields.h>
#include <valueAdd/protocol/ErrorCodes.h>
#include <valueAdd/protocol/Feature.h>
#include <valueAdd/protocol/SystemParameters.h>
#include <valueAdd/protocol/types.h>
...
private:
    ...
    static Json::Value jvParseCurrencyIssuer (std::string const& strCurrencyIssuer)
    {
        ...
        if (boost::regex_match (strCurrencyIssuer, smMatch, reCurIss))
        {
            ...
            if (strIssuer.length ())
            {
                // Could confirm issuer is a valid valueAdd address.
...
private:
    ...
    bool isValidJson2(Json::Value const& jv)
    {
        if (jv.isObject())
        {
            if (jv.isMember(jss::jsonrpc) && jv[jss::jsonrpc] == "2.0" &&
                jv.isMember(jss::vacrpc) && jv[jss::vacrpc] == "2.0" &&
    ...
    Json::Value parseJson2(Json::Value const& jvParams)
    {
        ...
        if (jv.isMember(jss::vacrpc))
            jv_error[jss::vacrpc] = jv[jss::vacrpc];
    ...
    Json::Value parseVacPathFind (Json::Value const& jvParams)
    {
        Json::Reader    reader;
        Json::Value     jvRequest;
...
public:
    ...
    //      {   "profile",              &RPCParser::parseProfile,               1,  9   },
            ...
            {   "vac_path_find",     &RPCParser::parseVacPathFind,        1,  2   },
...
cmdLineToJSONRPC (std::vector<std::string> const& args, beast::Journal j)
{
    ...
    if (paramsObj.isMember(jss::vacrpc))
        jv[jss::vacrpc] = paramsObj[jss::vacrpc];
    ...
    Config const& config, Logs& logs)
{
    ...
    if (jvRequest.isMember(jss::vacrpc))
        jvOutput[jss::vacrpc] = jvRequest[jss::vacrpc];
